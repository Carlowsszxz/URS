        ...
        async function loadTriviaLeaderboard(filter = 'all-time') {
            console.log('Loading trivia leaderboard, filter:', filter);
            
            try {
                // Query trivia_scores table directly
                let query = window.supabaseClient
                    .from('trivia_scores')
                    .select('*')
                    .order('score', { ascending: false })
                    .order('created_at', { ascending: true })
                    .limit(500); // Get more to filter later
                ...
                console.log('Raw trivia scores:', data);
                ...
                // Try to fetch user names from users table
                ...
                // Add user info to each score
                const scoresWithUsers = uniqueScores.map(score => {
                ...
                console.log('Final leaderboard with user info:', scoresWithUsers);
                return scoresWithUsers;
                
            } catch (error) {
                console.error('Exception in loadTriviaLeaderboard:', error);
                return [];
            }
        }
        ...
        async function loadLeaderboardManual(filter = 'all-time') {
            try {
                ...
                // Fetch user profiles from users table (if exists)
                    ...
                    <div class="leaderboard-entry ${isCurrentUser ? 'current-user' : ''}" data-rank="${rank}">
                        ...
                        <div class="entry-info">
                            ...
                            <div class="entry-stats">
                                ...
                                <span class="entry-stat">
                                    <i data-lucide="clock"></i> ${Math.floor(entry.time_taken / 60)}:${(entry.time_taken % 60).toString().padStart(2, '0')}
                                </span>
                                <span class="entry-stat">
        ...
        async function saveToLeaderboard() {
            try {
                ...
                if (sessionError || !session) {
                    console.error('No active session, cannot save score');
                    updateRankDisplay('Unable to save score - Please log in');
                    return;
                }
                ...
                let playerName = userEmail.split('@')[0];
                try {
                    ...
                    console.log('Could not fetch profile name, using email prefix');
                }
                ...
                const scoreData = {
                    ...
                    game_name: 'Timeline Quest',
                    score: timelineGameState.score,
                    perfect_rounds: timelineGameState.perfectRounds,
                    total_rounds: timelineGameState.totalRounds,
                    accuracy: accuracy,
                    time_seconds: Math.floor(totalTime / 1000),
                    era_selected: timelineGameState.selectedEra,
                ...
                if (error) {
                    ...
                    updateRankDisplay('Score saved locally only');
                    return;
                }
                ...
                // Get player rank
                await getPlayerRank(userId, timelineGameState.score);
                
            } catch (error) {
                console.error('Error in saveToLeaderboard:', error);
                updateRankDisplay('Unable to save score');
            }
        }
        
        // Get Player Rank
        async function getPlayerRank(userId, score) {
            try {
                const supabaseClient = window.supabaseClient || await getSupabaseClient();
                
                // Get count of scores higher than player's score for Timeline Quest
                const { count, error } = await supabaseClient
                    .from('game_leaderboard')
                    .select('*', { count: 'exact', head: true })
                    .eq('game_name', 'Timeline Quest')
                    .gt('score', score);
                
                if (error) {
                    console.error('Error getting rank:', error);
                    updateRankDisplay('Score saved successfully!');
                    return;
                }
                ...
                updateRankDisplay(`You ranked ${rank}${suffix} place!`);
        ...
        // Update Rank Display
        function updateRankDisplay(message) {
            const rankDisplay = document.getElementById('timelineRankDisplay');
            if (rankDisplay) {
        ...
        async function loadTimelineLeaderboard(filter = 'all-time') {
            ...
            try {
                ...
                if (!data || data.length === 0) {
                    leaderboardList.innerHTML = `
                        <div class="timeline-empty-leaderboard">
                            <i data-lucide="trophy"></i>
                            <p>No scores yet. Be the first to play!</p>
                        </div>
                    `;
                ...
                data.forEach((entry, index) => {
                    ...
                    html += `
                        <div class="timeline-leaderboard-entry ${rankClass} ${userClass}">
                            ...
                            <div class="entry-info">
                                ...
                                <div class="entry-stats">
                                    <span><i data-lucide="target"></i> ${entry.accuracy}%</span>
                                    <span><i data-lucide="clock"></i> ${timeStr}</span>
                                    <span><i data-lucide="check-circle"></i> ${entry.perfect_rounds}/${entry.total_rounds}</span>
        ...
        const difficultyLevels = {
            ...
            medium: {
                ...
                timeBonus: 500,
                baseScore: 400,
                description: "6 Events • Mixed Eras"
            },
            hard: {
                name: "Hard",
                eventCount: 8,
